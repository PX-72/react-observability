...
describe("fetchWithTrace", () => {
  it("injects traceparent header and returns it", async () => {
    ...
    const result = await fetchWithTrace("/api/requests", { method: "POST" }, { traceparent });
  ...
  it("parses text response when not JSON", async () => {
    ...
    const { data, response } = await fetchWithTrace("/api/requests", {}, { traceparent: "00-0123456789abcdef0123456789abcdef-0123456789abcdef-01" });
